...
const Page: NextPage = () => {
  ...
  const { data, error } = useSWR<ApiResponseSuccess<IQuestion>, ApiResponseError>(
    query.id && locale
      ? {
          method: 'GET',
      ...
      <div className="mx-auto max-w-screen-md py-20">
        ...
        <div className="my-8 text-center font-heading">
          <h2 className="mb-2 text-xl font-normal">
            {strSubstitute(t('QUEST_NUMBER_TITLE', { ns: 'activity' }), data.payload.questNo)}
          </h2>
          <h1 className="text-3xl font-bold drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]">{data.payload.questTitle}</h1>
        </div>

        <img src="/static/images/nstar/nstar_atomic_done.svg" className="mx-auto pl-10 h-40 max-w-sm drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]" alt="Done" />

        <div className="mt-10 w-full space-y-5 text-center">
          <div className="font-heading text-3xl font-bold drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]">
            {t('QUEST_RESULT.DONE_TITLE', { ns: 'activity' })}
          </div>
          <div className="font-heading text-2xl drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]">{t('QUEST_RESULT.DONE_SUBTITLE', { ns: 'activity' })}</div>
        </div>
